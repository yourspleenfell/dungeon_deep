...
def treasure(request, floor, room):
    ...
    if event_chance > 1:
        gold = random.randint(1, 7) + int(floor)
        char.gold += gold
        char.save()
        request.session['log'].append('You have found ' + str(gold) + ' gold in the treasure chest!')
    if int(room) is 10:
        return redirect(reverse('dungeon:dungeon', kwargs={'floor': int(floor) + 1, 'room': 1}))
    else:
        request.session['treasure_clicked'] = False
...
def battle(request, floor, room):
    ...
    if not result[1][0]:
        request.session['log'].append(request.session['monster_name'] + ' missed!')
    else:
        request.session['log'].append(request.session['monster_name'] + ' damaged you for ' + str(result[1][1]))
    if not result[2][0]:
        request.session['log'].append('You missed!')
    else:
        request.session['log'].append('You have damaged ' + request.session['monster_name'] + ' for ' + str(result[2][1]))   
    if request.session['current_vitality'] <= 0:
        request.session['log'].append('You have earned ' + str(result[3]) + ' experience!')
        if int(room) == 10:
            floor = int(floor) + 1
            room = 1
        else:
